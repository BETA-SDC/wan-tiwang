...
Feedback comes from exported slide decks. In an exported deck, players click options or use `A-D` / `1-9`, confirm with `Confirm Answer` or `Enter`, then click `Download Feedback`.
...
反馈来自导出的演示。玩家在演示中点击选项，或用 `A-D` / `1-9` 选择选项，再点击 `Confirm Answer` 或按 `Enter` 确认，最后点击 `Download Feedback` 下载反馈 JSON。
