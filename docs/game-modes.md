# Game Modes

Suggested game modes:

万题王的重点是玩法和场景。题目不仅要能被分类，还要能被组合成不同的游玩模式。

- `daily`: short daily trivia.
- `party`: easygoing group play.
- `stream`: audience voting and fast reveals.
- `challenge`: higher difficulty streak play.
- `themed`: focused topic rounds.
- `chaos`: mixed categories with surprising or funny questions.

Game modes should be expressed through packs, playlists, moods, occasions, and sampling rules.

中文说明：

- `daily`：每日一题或每日十题，适合轻量打开。
- `party`：朋友聚会，题目要轻松、好讨论。
- `stream`：直播互动，适合投票、弹幕竞猜和快速揭晓。
- `challenge`：挑战模式，可以更难、更适合连胜机制。
- `themed`：主题局，比如“奇怪历史”“太空冷知识”“游戏老玩家”。
- `chaos`：混合随机局，强调反差、惊喜和娱乐性。

玩法不要写死在题目里。更推荐通过 `packs/`、`playlists/`、`mood`、`occasion` 和抽样规则组合出来。
